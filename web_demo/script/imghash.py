...
from web_demo.common.common import init, img_download
...
def scheduler(db, category):
    ...
    with ThreadPoolExecutor() as executor:
        for item in source.find(
                {'platform': {'$nin': blacklist}, '$or': [{'is_hash': False}, {'is_hash': {'$exists': False}}]},
                {'platform': 1, '_id': 1, 'image': 1, 'system_id': 1}):
        ...
        try:  # avoid defunct process
            for index, url in enumerate(item['image']):
                ...
                if img.width > 150 and img.height > 150 and len(his) > 6:
                    ...
                    target.update_one({'imageurl': url},
                                      {'$setOnInsert': {'phash': str(hash_code), 'system_id': item['system_id']}},
                                      upsert=True)  # dont need to add imageurl into setOnInsert
                os.remove(picture)
            source.update_one({"_id": item['_id']},
                              {"$set": {"is_hash": True}})  # Put here to prevent missing some phash generation failures
