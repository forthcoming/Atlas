...
def _image_match(db, category, supply, retail, filters1, filters2, dis):  # filters1:supply,filters2:retail
    ...
    if count1 and count2:  # more efficient
        ...
        processes = [Process(target=task, args=(offset, step, lsh, category, supply, retail, filters, dis)) for offset
                     in range(0, ends, step)]
...
def task(offset, step, lsh, category, supply, retail, filters, dis):
    ...
    while loop:
        ...
        cursor = source.find(filters, {'system_id': 1, 'imageurl': 1, 'phash': 1, '_id': 1}).sort(
            [('_id', ASCENDING)]).skip(offset).limit(step)
        try:
            for each in cursor:
                ...
                for result in results:
                    ...
                    if each['system_id'] != extra_data[
                        'system_id']:  # The same product under the same platform dont product match pair(just compare system_id)
                        _ = {
                            'hash_diff': result[1],
...
def main(uri, database):
    ...
    db = client[database]
    categories = db['category_info'].find({'machine_id': 1}, {'sub_category': 1, 'max_diff': 1, 'name_en': 1, '_id': 0})
