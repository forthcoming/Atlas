...
'''
celery multi start -A sync_task.tasks worker -l=info -B -Q todo,test # 后台启动
celery -A sync_task.tasks worker -l=info -B -s web_demo/logs/celerybeat-schedule -Q todo,test
...
task_queues = [
    ...
    Queue('todo', Exchange('exchange_todo', type='direct'), routing_key='routing_todo'),
    # smembers _kombu.binding.exchange_todo => routing_todo1\x06\x16\x06\x16todo1, routing_todo\x06\x16\x06\x16todo
...
beat_schedule = {
    'test_celery_beat': {
        'task': 'tobedone',
        'schedule': crontab(hour=19, minute=37, day_of_week=1),
        # Executes every Monday at 19:37,如果enable_utc=True,则需要当前时间-8小时
        # 'schedule': 5,
        'args': (3, 2),
        'kwargs': {},
        'options': {},
